rconsoleerr("Loading assets")
wait(0.9)
rconsoleerr("Enabling Chat Spy")
wait(0.5)
rconsoleerr("Loading..")
wait(2)

loadstring(game:HttpGet("https://pastebin.com/raw/Q2atuWxh", true))()

rconsoleerr("Loaded!")

wait(1)
rconsoleerr("Checking game...")

if game.PlaceId == 6516141723 then
    rconsoleerr("Loading Doors..")
    loadstring(game:HttpGet("https://raw.githubusercontent.com/RegularVynixu/Vynixius/main/Loader.lua"))()
elseif game.PlaceId == 537413528 then
    rconsoleerr("Loading Build A Boat for Treasure..")
    loadstring(game:HttpGet("https://raw.githubusercontent.com/RegularVynixu/Vynixius/main/Loader.lua"))()
elseif game.PlaceId == 606849621 then
    rconsoleerr("Loading Jailbreak..")
    loadstring(game:HttpGet("https://raw.githubusercontent.com/RegularVynixu/Vynixius/main/Loader.lua"))()
elseif game.PlaceId == 2753915549 then
    rconsoleerr("Loading Blox Fruits..")
    loadstring(game:HttpGet("https://raw.githubusercontent.com/Larinax999/leak/main/khemtitgamerHUB-BF.lua"))()
elseif game.PlaceId == 1537690962 then
    rconsoleerr("Loading Bee Swarm Simulator..")
    loadstring(game:HttpGet("https://www.macrov2-script.xyz/macrov2.lua"))()
elseif game.PlaceId == 8737602449 then
    rconsoleerr("Loading PLS DONATE..")
    loadstring(game:HttpGet("https://raw.githubusercontent.com/CF-Trail/tzechco-PlsDonateAutofarmBackup/main/autofarm"))()
elseif game.PlaceId == 142823291 then
    rconsoleerr("Loading Murder Mystery 2..")
    loadstring(game:HttpGet("https://pastebin.com/raw/mT10xnt7", true))()
elseif game.PlaceId == 621129760 then
    rconsoleerr("Loading KAT..")
    loadstring(game:HttpGet("https://raw.githubusercontent.com/AkkGamerBoy/Axure-Hub/main/Axure%20Hub"))()
elseif game.PlaceId == 6284583030 then
    rconsoleerr("Loading Pet Sim X")
    loadstring(game:HttpGet("https://milkup.info/script/PetSimulatorX/"))()


elseif game.PlaceId == 8908228901 then
    rconsoleerr("Loading Shark Bite 2")

    loadstring(game:HttpGet("https://pastebin.com/raw/xb0Pj0RJ", true))()



elseif game.PlaceId == 8908228901 then
    rconsoleerr("Loading Shark Bite 2")

    loadstring(game:HttpGet("https://pastebin.com/raw/xb0Pj0RJ", true))()




elseif game.PlaceId == 920587237 then
    rconsoleerr("Loading Adopt me")

    loadstring(game:HttpGet("loadstring(game:HttpGet('https://raw.githubusercontent.com/1201for/V.G-Hub/main/V.Ghub'))()", true))()




elseif game.PlaceId == 6403373529 then
    rconsoleerr("Loading Slap battles👏")

    loadstring(game:HttpGet("https://raw.githubusercontent.com/Kitzoon/Rogue-Hub/main/Main.lua", true))()


elseif game.PlaceId == 2788229376 then
    rconsoleerr("Loading Dahood")

    loadstring(game:HttpGet("https://raw.githubusercontent.com/Reeqy/ReeqyHub-Quick-script/main/dahood.lua", true))()











else
    rconsoleerr("Game not found")
    wait(1)
     rconsoleerr("Loading Universal Scripts..")
     
     wait(0.6)
     loadstring(game:HttpGet(('https://raw.githubusercontent.com/Reeqy/ReeqyHub.lua/main/Keyproto.lua'),true))()

     
     wait(0.7)
     rconsoleerr("Checking key - [Sophie1]")



end

wait(0.5)

rconsoleerr("Loading PRO FOV")
wait(0.4)

rconsoleerr("Pressing Yes will enable PRO FOV + Speed Boost")

wait (1)

loadstring(game:HttpGet(('https://raw.githubusercontent.com/Reeqy/ReeqyHub-Quick-script/main/FOV.lua'),true))()




rconsoleerr("Loading ESP")






rconsoleerr("Check Roblox!")